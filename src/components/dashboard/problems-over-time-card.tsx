...
// 	count: number; // Number of problems solved on that date (running total)
// }
...
// 			)
// 			console.log('acceptedSubmission: ', acceptedSubmissions[0])
...
//             // Handle case where submissions might be null/undefined but not loading/erroring
...
// 	// Get min and max values for Y-axis domain based on processed data
// 	const getYAxisDomain = (data: ProblemsSolvedDataPoint[]) => {
// 		if (!data || data.length === 0) return [0, 10] // Keep default if no data
// 		// Calculate min/max based on actual counts, not running total for better axis scaling
// 		const counts = data.map(d => d.count) 
// 		// If only one data point, set a sensible range
//         if (counts.length === 1) {
//             return [0, Math.max(10, counts[0] + 3)];
//         }
// 		const minCount = Math.max(0, Math.min(...counts) - 3)
...
// 				{/* Update loading/error/data checks */}
// 				{displayLoading ? (
...
// 							/>
// 							<YAxis 
// 								// Use processed data for domain
// 								domain={getYAxisDomain(problemsOverTimeData)} 
// 								tick={{ fill: '#D1D5DC', fontSize: 12 }}
// 								tickLine={{ stroke: '#D1D5DC' }}
// 								axisLine={{ stroke: '#D1D5DC' }}
// 								// Allow decimals if needed for smaller ranges
//                                 allowDecimals={false} 
// 							/>
